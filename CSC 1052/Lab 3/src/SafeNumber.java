...
public class SafeNumber
{
	...
	public SafeNumber(int n) throws GreaterThanMaxException, LessThanMinException
	{
		if(n > MAX) {
			number = 0;
			throw new GreaterThanMaxException("Number " + n + " is greater than " + MAX);
			
		}
		else if(n < MIN) {
			number = 0;
			throw new LessThanMinException("Number " + n + " is less than " + MIN);
		}
		number = n;
	...
	{
		if(number - n < MIN) {
			throw new LessThanMinException("result " + (number - n) + " is greater than MIN " + MIN);
		}
		number -= n;
	}
}
